...
class UserToDeliveryBoyChatScreen extends HookWidget {
  final String? orderId;
  final int? orderID;
  UserToDeliveryBoyChatScreen({this.orderId, this.orderID});
  ...
  Widget build(BuildContext context) {
    ...
    final state = useProvider(userToDeliveryBoyChatProvider);
    final isMounted = useIsMounted();
    useEffect(() {
      if (isMounted()) {
        Future.delayed(Duration.zero, () {
          context
              .read(userToDeliveryBoyChatProvider.notifier)
    ...
    return Scaffold(
      ...
      appBar: titleAppbar(context, 'Chat : ${orderID ?? ""}'),
              ...
              children: <Widget>[
                Column(
                  children: <Widget>[
                    ...
                    Divider(height: 1.0),
                    (ORDER_STATUS.delivered == state.chatData!.orderStatus!)
                        ? Container()
                        : Container(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () {
                                          var chatRequest =
                                              UserToDeliveryBoyChatMessageReponse(
                                                  orderId: state.chatData?.id,
                                                  deliveryBoyId:
                                                      state
                                                          .chatData?.deliveryBoyId,
                                                  deliverBoyName: state
                                                      .chatData?.deliverBoyName,
